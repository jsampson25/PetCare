...
Migration `20260718001200_refund_ledger.sql` adds permission-controlled full and partial refunds without rewriting original payments. A successful refund creates an immutable refund, matching invoice credit, refund receipt, and transactional delivery event. Stripe refunds use connected-account API calls, provider idempotency, metadata correlation, and verified refund-event processing; manual tenders use the same ledger finalization after staff confirms money was returned outside PetCare. Refundable amounts are derived from the original settled payment less prior refunds, and invoice balances separately retain gross charges, credits, payments, and returned funds.

Migration `20260718001300_transactional_email_delivery.sql` turns the financial-message outbox into a recoverable delivery pipeline. A protected server worker atomically claims due email work, renders escaped invoice/payment/refund content, sends through Resend without placing provider credentials in the browser, and records provider IDs only after acceptance. Failures use bounded exponential retry and stop auto-claiming after five attempts; payment managers can inspect and explicitly requeue failures without changing the underlying invoice, booking, payment, or refund. Delivery failure therefore remains operationally visible while never rolling back valid financial state.

Migration `20260718001400_dispute_reconciliation.sql` adds Stripe dispute correlation and an operator-owned exception queue. Signature-verified dispute events map through the provider PaymentIntent to exactly one tenant payment, retain amount, currency, reason, lifecycle status, and evidence deadline, and produce a reconciliation finding without rewriting the original payment. Unknown payment references are quarantined; amount and currency discrepancies receive distinct findings. Payment managers can document resolution while the complete dispute and webhook histories remain intact and duplicate processing returns the original dispute.

Migration `20260718000900_merchant_webhook_inbox.sql` and `/api/webhooks/stripe` establish the untrusted-provider boundary. Each business maps to at most one Stripe connected-account reference, and each provider account maps globally to exactly one PetCare tenant. The stored controller metadata makes dashboard access, requirements collection, fee responsibility, and loss responsibility explicit; the MVP default keeps identity requirements and connected-account losses with Stripe. Account references are written only by the service-role onboarding boundary, begin pending, and cannot claim charge or payout capability until synchronized from Stripe. Tenant users cannot type or self-assert an account mapping.

The webhook endpoint verifies Stripe's signature against the untouched raw UTF-8 request body, enforces a five-minute timestamp tolerance, hashes the payload, and persists the event through a service-role-only function before acknowledging it. Provider event IDs are globally idempotent. Mapped events enter the pending queue; unknown or inactive merchant accounts are quarantined without heuristic tenant attachment. Raw verified payloads live in a separate table that tenant roles cannot read, while owners can inspect safe event metadata in Payment settings. Processing remains asynchronous and will be added after payment-intent contracts are implemented.

Implementation references: [Stripe webhook signature verification](https://docs.stripe.com/webhooks/signature), [Stripe webhook delivery](https://docs.stripe.com/webhooks), and [connected-account controller configuration](https://docs.stripe.com/connect/accounts-v2/connected-account-configuration).

Migration `20260718001000_stripe_hosted_onboarding.sql` adds explicit owner-initiated Stripe-hosted onboarding. PetCare creates the connected account server-side with an idempotency key, immediately stores only its safe account reference and controller/capability metadata through a service-role-only synchronization function, and redirects the authenticated owner to a single-use Account Link. The refresh path creates a new link rather than reusing an expired URL. The return path retrieves provider truth because returning does not mean onboarding is complete. Online capability becomes active only when Stripe reports both charges and payouts enabled; outstanding requirements and disabled reasons remain visible and actionable. Platform secret keys stay server-only.

Migration `20260718001100_stripe_checkout_posting.sql` adds Stripe-hosted invoice collection without exposing card data to PetCare. A tenant-scoped, bounded payment request is created only for an active merchant and outstanding invoice balance. Stripe Checkout runs as a direct connected-account payment and carries only the PetCare payment-request correlation ID. The signed webhook enters the durable inbox before best-effort posting. Posting rechecks tenant, merchant, session, amount, currency, invoice, and paid status; duplicates return the original payment. A valid completion creates one payment, allocation, receipt, outbox message, invoice transition, and—when the required deposit is fully allocated—uses Booking's existing confirmation orchestration. Posting failures remain pending for retry instead of causing Stripe delivery rejection.
