...
The expanded E05 slice attaches structured booking questions and eligibility requirements to published version identities and exposes staff configuration on the same Services screen. Questions support text, yes/no, select, date, and numeric answers with explicit required state. Requirements distinguish hard blocks, staff review, and warnings and preserve the customer explanation that later booking flows will display.

Published question and requirement sets are now immutable alongside their service version. Managers create a draft revision that copies the active definition, questions, and requirements; they revise the draft and publish it as the next numbered version, leaving prior bookings interpretable against the superseded version.
