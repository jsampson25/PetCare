...
export default async function SecuritySettingsPage({ searchParams }: { searchParams: SearchParameters }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? <Alert title="Session action failed" tone="danger">{error}</Alert> : null}
      {notice ? <Alert title="Session updated" tone="success">{notice}</Alert> : null}
      ...
      <Card description="Review this session or revoke access on other devices. PetCare never displays or stores session tokens." title="Sessions">
        <dl className="space-y-3 text-sm">
          <div><dt className="font-bold">Authentication</dt><dd>{session.assurance}</dd></div>
          <div><dt className="font-bold">Current token issued</dt><dd>{session.issuedAt ? dateFormatter.format(session.issuedAt) : 'Current session'}</dd></div>
          <div><dt className="font-bold">Access token refresh by</dt><dd>{session.expiresAt ? dateFormatter.format(session.expiresAt) : 'Managed securely'}</dd></div>
        </dl>
        <div className="mt-6 flex flex-wrap gap-3">
          <form action={signOutOtherSessions}><Button type="submit" variant="secondary">Sign out other devices</Button></form>
          <form action={signOutEverywhere}><Button type="submit" variant="danger">Sign out everywhere</Button></form>
        </div>
      </Card>
    </div>
  );
